...
class ScriptCompilerTest {
    ...
    fun compile_basicScript_generatesValidLua() {
        ...
        assertTrue(luaCode.contains("events_state[\"AutoLogin\"].timer = events_state[\"AutoLogin\"].timer + dt"))
        assertTrue(luaCode.contains("if events_state[\"TimeoutCheck\"].timer >= 10.0 then"))
