...
fun DetailedScriptEditor(
    ...
    val lines = remember { mutableStateListOf<ParsedSimpleLine>() }
    LaunchedEffect(config.id, config.steps) {
        if (lines.toList() != config.steps) {
            lines.clear()
            lines.addAll(config.steps)
        }
    }

    fun pushSteps() {
        onUpdateConfig(
            config.copy(steps = lines.toList())
        )
            ...
            item {
                Section(name = "基本資訊") {
                    NoPaddingOutlinedTextField(
                        value = config.name,
                        onValueChange = { v -> onUpdateConfig(config.copy(name = v)) },
                        label = "名稱",
                    )
                    NoPaddingOutlinedTextField(
                        value = config.description,
                        onValueChange = { v -> onUpdateConfig(config.copy(description = v)) },
                        label = "描述",
                        singleLine = false,
                    )
                }
                ...
                Section(name = "執行策略") {
                    MacroLoopEditor(
                        ...
                        onLoopModeChange = { mode ->
                            onUpdateConfig(config.copy(loopMode = mode))
                        },
                    )
                }
            }
            ...
            itemsIndexed(items = lines, key = { i, _ -> "step-$i" }) { index, line ->
                ...
                Box(
                    ...
                    val parsedResult = runCatching {
                        line
                        // TODO: add support for parsed failed line
                    }
                    if (parsedResult.isSuccess) {
                        ...
                        SimpleScriptRawLine(
                            index = index,
                            rawLine = line.encodeToLine(),
                            dragHandleModifier = dragModifier,
                            errorHint = hint,
                            onRawLineChange = {
                                lines[index] = runCatching {
                                    parseSimpleScriptLine(it)
                                }.getOrNull() ?: return@SimpleScriptRawLine
                                pushSteps()
                            },
                            onDelete = {
                                lines.removeAt(index)
