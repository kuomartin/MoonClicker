            ...
            Column(
                ...
                when (selectedType) {
                    ActionTypeEnum.CLICK -> {
                        ...
                        Button(onClick = { /* TODO: Trigger Point Selection */ }, modifier = Modifier.fillMaxWidth()) {
                    ...
                    ActionTypeEnum.SWIPE -> {
                        ...
                        Button(onClick = { /* TODO: Trigger Point Selection for p1 */ }, modifier = Modifier.fillMaxWidth()) { Text("Select Start Point") }
                        Text("End Point")
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Button(onClick = { /* TODO: Trigger Point Selection for p2 */ }, modifier = Modifier.fillMaxWidth()) { Text("Select End Point") }
                        OutlinedTextField(value = durationMs, onValueChange = { durationMs = it }, label = { Text("Duration (ms)") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), modifier = Modifier.fillMaxWidth())
                    }
                    ActionTypeEnum.WAIT -> {
                        ...
                        // Note: Skipping unit dropdown for brevity in this snippet, defaulting to MS.
                        // Can add similar ExposedDropdownMenuBox as in Condition
                    }
