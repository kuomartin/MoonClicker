...
To avoid modifying `MoonClickerService` (the Shizuku-hosted process) and to ensure WebRTC readiness:
1. We will use the Java `MediaCodec` API in the App process (`:engine` module) to construct a `H264EncoderSink`.
2. This sink requests an `InputSurface` from `MediaCodec` and registers it directly to `MoonClickerService.addVirtualDisplaySurface()`.
3. The internal `GlesDistributor` inside `MoonClickerService` seamlessly fans out rendering to this surface via EGL, achieving zero-copy transfer without dragging MediaCodec responsibilities into the native or privileged layers.
