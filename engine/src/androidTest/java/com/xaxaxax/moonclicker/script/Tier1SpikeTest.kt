...
class Tier1SpikeTest {
    ...
    /** `MoonClickerService` 用自稱 `com.android.shell` 的 Context 建顯示器，在測試進程裡也要成立。 */
    @Test
    ...
    fun step2b_each_privileged_flag_follows_its_own_permission() {
        ...
        val granted = env.context.checkSelfPermission(MoonClickerService.ADD_TRUSTED_DISPLAY) ==
        ...
        val unlockedGranted = env.context.checkSelfPermission(
            MoonClickerService.ADD_ALWAYS_UNLOCKED_DISPLAY
        ) == PackageManager.PERMISSION_GRANTED
        assertEquals(
    ...
    fun step5_an_injected_tap_reaches_the_puppet() {
        ...
        assertNotNull(
            "no ACTION_DOWN reached the puppet on display $displayId at all\n" +
                    ...
                    "\n--- logcat ---\n" +
                    env.logcat("MoonClickerService", "InputManager", "InputDispatcher", "InputReader"),
            probe.last,
        )
        assertTrue(
