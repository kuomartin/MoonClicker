 ...
 * 睡著之後，注入的輸入要能把它喚醒——修法前這裡會卡死（見 [MoonClickerService.wakeDisplayGroupIfOwned]）。
 *
...
class VirtualDisplayIdleDeadlockTest {
     ...
     * `state ON/OFF` 在更早的 `DisplayDeviceInfo{"moonclicker-tier1"...}` 那一段裡，只能用建立時的
     * 顯示器名稱去找。
     */
    private fun powerState(): String {
        val all = env.shell("dumpsys display")
        val start = all.indexOf("DisplayDeviceInfo{\"moonclicker-tier1\"")
        check(start >= 0) { "moonclicker-tier1 not found in dumpsys display:\n$all" }
